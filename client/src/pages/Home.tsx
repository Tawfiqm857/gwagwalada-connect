...
export default function Home() {
  ...
  const peopleInput = useMemo(() => user?.id ? { viewerId: user.id } : undefined, [user?.id]);
  const peopleQuery = trpc.social.people.useQuery(peopleInput);
  const relationshipsQuery = trpc.social.relationships.useQuery(undefined, { enabled: isAuthenticated });
  const followMutation = trpc.social.follow.useMutation();
  const friendRequestMutation = trpc.social.friendRequest.useMutation();
  ...
  const followedPeople = useMemo(() => (relationshipsQuery.data?.followingIds ?? []).map(String), [relationshipsQuery.data?.followingIds]);
  const friendRequests = useMemo(() => Object.fromEntries((relationshipsQuery.data?.friendRequests ?? []).map((request) => [String(request.userId), request.status])), [relationshipsQuery.data?.friendRequests]);
  const incomingRequests = useMemo(() => (relationshipsQuery.data?.friendRequests ?? []).filter((request) => request.direction === "incoming" && request.status === "pending").map((request) => String(request.userId)), [relationshipsQuery.data?.friendRequests]);
  ...
  const toggleFollow = (personId: string, name: string) => {
    if (!isAuthenticated) { window.location.href = "/auth"; return; }
    const follow = !followedPeople.includes(personId);
    followMutation.mutate({ userId: personId, follow }, { onSuccess: async () => { await relationshipsQuery.refetch(); toast.success(follow ? `You’re now following ${name}.` : `You unfollowed ${name}.`); }, onError: (error) => toast.error(error.message) });
  };
  const updateFriendRequest = (personId: string, action: "send" | "accept" | "decline" | "cancel", name: string) => {
    if (!isAuthenticated) { window.location.href = "/auth"; return; }
    friendRequestMutation.mutate({ userId: personId, action }, { onSuccess: async (result) => { await relationshipsQuery.refetch(); toast.success(result.status === "accepted" ? `${name} is now a connection. You can message each other.` : action === "send" ? `Friend request sent to ${name}.` : action === "cancel" ? `Friend request to ${name} cancelled.` : `Friend request with ${name} updated.`); }, onError: (error) => toast.error(error.message) });
  };
    ...
    <div className="app-shell">
      ...
      <div className="workspace">
        ...
        <main className="main-content">
          ...
          {activeView === "people" && <PeopleView people={discoveredPeople} followedPeople={followedPeople} friendRequests={friendRequests} incomingRequests={incomingRequests} onFollow={toggleFollow} onFriendRequest={updateFriendRequest} onMessage={(person) => { setSelectedThread({ name: person.name, initials: person.initials, preview: "New connection", time: "Now", unread: 0, tone: person.tone }); setActiveView("messages"); }} />}
        </main>
      </div>
...
function PeopleView({ people, followedPeople, friendRequests, incomingRequests, onFollow, onFriendRequest, onMessage }: { people: Person[]; followedPeople: string[]; friendRequests: Record<string, "pending" | "accepted" | "declined">; incomingRequests: string[]; onFollow: (personId: string, name: string) => void; onFriendRequest: (personId: string, action: "send" | "accept" | "decline" | "cancel", name: string) => void; onMessage: (person: Person) => void }) {
